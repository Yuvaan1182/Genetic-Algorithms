...
public:
    ...
    long double ackley(vector<long double> sol)
    {
        long double res = 0;
    ...
    void calculateFitness()
    {
        ...
        {
            long double fit_val = evaluateFitness(ackley(population[i]));
    ...
    void employee()
    {
        for (int i = 0; i < population_size; i++)
        {
            ...
            long double new_fitness = this->evaluateFitness(this->ackley(new_solution));
    ...
    void onlooker()
    {
        ...
        while (i < population_size)
        {
            ...
            if (rnd <= prob[i])
            {
                ...
                auto new_fitness = this->evaluateFitness(this->ackley(new_solution));
                if (new_fitness > fitness[i])
                {
                    population[i] = new_solution;
                    fitness[i] = new_fitness;
...
int main()
{
    ...
    for (int i : best.second)
        cout << i << " ";
    cout << endl;
    cout << (abc->ackley(best.second)) << endl;
    return 0;
}
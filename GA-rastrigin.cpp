...
public:
    ...
    // rastrigin function => fitness value calculation for each chromosome
    double rastrigin(vector<double> sol)
    {
        double res = A * chromosome_size;
    ...
    void mutateCrossOverPopulation(vector<vector<double>> &crossover_pop, int crossover_pop_size)
    {
        for (int i = 0; i < crossover_pop_size; i++)
        {
            ...
            double mutated_child_fitness = 1 / rastrigin(child);
            double parent_fitness = 1 / rastrigin(population[i]);
    ...
    void roulleteWheel()
    {
        ...
        for (int i = 0; i < population_size; i++)
        {
            double val = rastrigin(population[i]);
            tot_val = val;
            fitness[i] = (1 / val);
        }

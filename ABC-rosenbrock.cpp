...
public:
    ...
    long double rosenbrockrigin(vector<long double> sol)
    {
        long double res = 0;
        for (int i = 0; i < dimension - 1; i++)
        {
            res += 100 * (sol[i + 1] - sol[i] * sol[i]) * (sol[i + 1] - sol[i] * sol[i]) + (1 - sol[i]) * (1 - sol[i]);
        }
    ...
    void calculateFitness()
    {
        ...
        {
            long double fit_val = evaluateFitness(rosenbrockrigin(population[i]));
    ...
    void employee()
    {
        for (int i = 0; i < population_size; i++)
        {
            ...
            long double new_fitness = this->evaluateFitness(this->rosenbrockrigin(new_solution));
    ...
    void onlooker()
    {
        ...
        while (i < population_size)
        {
            ...
            if (rnd <= prob[i])
            {
                ...
                auto new_fitness = this->evaluateFitness(this->rosenbrockrigin(new_solution));
                if (new_fitness > fitness[i])
                {
                    population[i] = new_solution;
                    fitness[i] = new_fitness;
...
int main()
{
    ...
    for (int i : best.second)
        cout << i << " ";
    cout << endl;
    cout << (abc->rosenbrockrigin(best.second)) << endl;
    return 0;
}
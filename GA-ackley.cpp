...
public:
    ...
    // ackley function => fitness value calculation for each chromosome
    ...
    void mutateCrossOverPopulation(vector<vector<double>> &crossover_pop, int crossover_pop_size)
    {
        for (int i = 0; i < crossover_pop_size; i++)
        {
            ...
            double mutated_child_fitness = 1 / ackley(child);
            double parent_fitness = 1 / ackley(population[i]);
    ...
    void roulleteWheel()
    {
        ...
        for (int i = 0; i < population_size; i++)
        {
            double val = ackley(population[i]);
            tot_val = val;
            fitness[i] = (1 / val);
        }
